...
struct thread_args {
  ...
  int fd;
  //file descriptor, last pipeline stage only
  int fd_out;
...
stats_t *threads_anchor_rv[conf_e->nthreads];
stats_t *threads_chunk_rv[conf_e->nthreads];
stats_t *threads_compress_rv[conf_e->nthreads];
...
static int create_output_file(char *outfile) {
  ...
  //Write header
  if (write_header(fd, conf_e->compress_type)) {
...
std::vector<chunk_t *> Compress(std::vector<chunk_t *> &v_in) {
  chunk_t * chunk;
  int r;
  std::vector<chunk_t *> v_out;


  fprintf(stderr, "%d %d\n", sizeof(v_in), v_in.size());
 ...
 int i = 0;
  ...
  while(i<v_in.size()) {
      //fetch one item
    chunk = (chunk_t *)v_in[i];
    assert(chunk!=NULL);
    if(chunk->header.compress){
...
int sub_Deduplicate(chunk_t *chunk) {
...
#ifdef ENABLE_PTHREADS
  pthread_mutex_t *ht_lock = (pthread_mutex_t*)hashtable_getlock(cache_e, (void *)(chunk->sha1));
  pthread_mutex_lock(ht_lock);
#endif
  entry = (chunk_t *)hashtable_search(cache_e, (void *)(chunk->sha1));
  ...
  if (!isDuplicate) {
    ...
    if (hashtable_insert(cache_e, (void *)(chunk->sha1), (void *)chunk) == 0) {
 ...
 */
std::vector<chunk_t *> Deduplicate(std::vector<chunk_t *> &v_in) {
  chunk_t *chunk;
  int r;
  ...
  while (i<v_in.size()) {
    ...
    if(!isDuplicate) {
      chunk->header.compress= 1;
    } else {
      chunk->header.compress= 0;
      }
...
std::vector<chunk_t *> FragmentRefine(chunk_t * v_in) {
  int r;
...
std::experimental::optional<chunk_t *> Fragment(){
  size_t preloading_buffer_seek = 0;
  int qid = 0;
  int fd = data_process_args->fd;
  int i;
  ...
  while (1){
	  if(temp_f != NULL) {
	    bytes_left = temp_f->uncompressed_data.n;
	  } else {
	    bytes_left = 0;
	  }
	  ...
	  if(bytes_left > 0) {
	    ...
	    chunk->sequence.l1num = temp_f->sequence.l1num;

	    //NOTE: We cannot safely extend the current memory region because it has already been given to another thread
	    memcpy(chunk->uncompressed_data.ptr, temp_f->uncompressed_data.ptr, temp_f->uncompressed_data.n);
	    mbuffer_free(&temp_f->uncompressed_data);
	    free(temp_f);
	    temp_f = NULL;
	  } else {
	  ...
	  size_t bytes_read=0;
	  //fprintf(stderr, "MAXBUF %d\n", MAXBUF);
	  if(conf_e->preloading) {
	      size_t max_read = MIN(MAXBUF, data_process_args->input_file.size-preloading_buffer_seek);
	      memcpy(chunk->uncompressed_data.ptr+bytes_left, data_process_args->input_file.buffer+preloading_buffer_seek, max_read);
	      bytes_read = max_read;
	  ...
	  do {
	    ...
	    if(ANCHOR_JUMP < chunk->uncompressed_data.n) {
	      ...
	      if(offset == 0) {
	        ...
	        //Allocate a new chunk and create a new memory buffer
	        temp_f = (chunk_t *)malloc(sizeof(chunk_t));
	        if(temp_f==NULL) EXIT_TRACE("Memory allocation failed.\n");

	        //split it into two pieces
	        r = mbuffer_split(&chunk->uncompressed_data, &temp_f->uncompressed_data, offset + ANCHOR_JUMP);
	        if(r!=0) EXIT_TRACE("Unable to split memory buffer.\n");
	        temp_f->header.state = CHUNK_STATE_UNCOMPRESSED;
	        temp_f->sequence.l1num = anchorcount;
	        anchorcount++;
	        //put it into send buffer
	        split = 1;
	        return chunk;
...
void *Reorder(std::vector<chunk_t *> &v_in) {
  int qid = 0;
  int fd = data_process_args->fd_out;
  ...
  while(i<v_in.size()) {
  	...
  	//fprintf(stderr, "i %d, nc %d, nc2 %d\n", i, next_c.l1num, next_c.l2num);
    ...
    if(!sequence_eq(chunk->sequence,next_c)) {
     //fprintf(stderr, "i %d, nc %d, nc2 %d\n", i, next_c.l1num, next_c.l2num);
      //fprintf(stderr, "chunk %d %d \n", chunk->sequence.l1num, chunk->sequence.l2num );
    ...
    do {
      ...
      sequence_inc_l2(&next_c);
      if(chunks_per_anchor[next_c.l1num]!=0 &&next_c.l2num==chunks_per_anchor[next_c.l1num]) sequence_inc_l1(&next_c);

      //Check whether we can write more chunks from cache
      if(pos != NULL && (pos->Element.l1num ==next_c.l1num)) {
...
void Encode(config_t * _conf) {
  ...
  int32 fd;
  int32 fd_out;
  data_process_args = (thread_args *) malloc(sizeof(thread_args));
  conf_e = _conf;
  const int nqueues = (conf_e->nthreads / MAX_THREADS_PER_QUEUE) +
                      ((conf_e->nthreads % MAX_THREADS_PER_QUEUE != 0) ? 1 : 0);
  ...
  //Create chunk cache
  cache_e = hashtable_create(65536, hash_from_key_fn, keys_equal_fn, FALSE);
  if(cache_e == NULL) {
  ...
  /* src file stat */
  if (stat(conf_e->infile, &filestat) < 0) 
      EXIT_TRACE("stat() %s failed: %s\n", conf_e->infile, strerror(errno));
  ...
  /* src file open */
  if((fd = open(conf_e->infile, O_RDONLY | O_LARGEFILE)) < 0) 
  ...
  fd_out = create_output_file(conf_e->outfile);
  sequence_reset(&next_c);

  data_process_args->tid = 0;
  data_process_args->nqueues = nqueues;
  data_process_args->fd = fd;
  data_process_args->fd_out = fd_out;
  ...
  //grppi::dynamic_execution ex = execution_mode((backend)thr, conf_e->nthreads);

  grppi::parallel_execution_native ex = grppi::parallel_execution_native(conf_e->nthreads*3 + 2);
  ex.set_queue_attributes(1024UL*1024, grppi::queue_mode::lockfree);
  //ex.disable_ordering();
 ...
 grppi::pipeline(ex,
  []()->std::experimental::optional<chunk_t*>{return Fragment();},
  
  grppi::farm(conf_e->nthreads,
  [](chunk_t * v_in){return FragmentRefine(v_in);}),
  grppi::farm(conf_e->nthreads,
  [](std::vector<chunk_t *> v_in){return Deduplicate(v_in);}),
  grppi::farm(conf_e->nthreads,
  [](std::vector<chunk_t *> v_in){return Compress(v_in);}),

  [](std::vector<chunk_t *> v_in){return Reorder(v_in);}
 ...
 pos = TreeFindMin(T);
  while(pos !=NULL) {
  	//fprintf(stderr, "%d\n", pos->Element.l1num);
    if(pos->Element.l1num == next_c.l1num) {
      chunk = FindMin(pos->Element.queue);
      if(sequence_eq(chunk->sequence, next_c)) {
        //Remove chunk from cache, update position for next iteration
        DeleteMin(pos->Element.queue);
...
#ifdef ENABLE_PARSEC_HOOKS
  __parsec_roi_end(conf_e->timefile);
#endif
  ...
  if (conf_e->infile != NULL)
    close(fd);

  assert(!mbuffer_system_destroy());

  hashtable_destroy(cache_e, TRUE);

#ifdef ENABLE_STATISTICS
  /* dest file stat */
  if (stat(conf_e->outfile, &filestat) < 0) 
      EXIT_TRACE("stat() %s failed: %s\n", conf_e->outfile, strerror(errno));
  stats.total_output = filestat.st_size;

  //Analyze and print statistics
  if(conf_e->verbose) print_stats(&stats);
#endif //ENABLE_STATISTICS
}
