 ...
 */
std::vector<chunk_t *> FragmentRefine(std::vector<chunk_t *> v_in) {
  int r;
...
void *SerialIntegratedPipeline(void * targs) {
  ...
  int fd = args->fd;
  int fd_out = create_output_file(conf_e->outfile);
  int r;
...
std::experimental::optional<vector<chunk_t *>> Fragment(){
  size_t preloading_buffer_seek = 0;
  int qid = 0;
...
void Encode(config_t * _conf) {
  ...
  struct thread_args anchor_thread_args[conf_e->nthreads];


  grppi::dynamic_execution ex = execution_mode((backend)seq, 1);
#ifdef ENABLE_PARSEC_HOOKS
    __parsec_roi_begin();
#endif
 ...
 grppi::pipeline(ex,
  []()->std::experimental::optional<std::vector<chunk_t*>>{return Fragment();},
  [](std::vector<chunk_t *> v_in){return FragmentRefine(v_in);},
