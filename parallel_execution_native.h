    ...
    std::size_t sequence_size, Transformer transform_op) const
{
  ...
  const auto offset = sequence_size % concurrency_degree_;
  
  {
    ...
    for (int i=0; i!=concurrency_degree_-1; ++i) {
      ...
      if(i<offset){
      	...
      	cs = chunk_size;
      	delta = (chunk_size + 1)*offset + (i-offset)*chunk_size;
      }
      const auto chunk_firsts = iterators_next(firsts,delta);
      const auto chunk_first_out = next(first_out, delta);
      workers.launch(*this, process_chunk, chunk_firsts, cs, chunk_first_out);
    }

    delta = (chunk_size+1)*offset + chunk_size * (concurrency_degree_ - 1 - offset);
    const auto chunk_firsts = iterators_next(firsts,delta);
    const auto chunk_first_out = next(first_out, delta);
    ...
    Combiner && combine_op) const
{
  ...
  
  {
    ...
    for (int i=0; i!=concurrency_degree_-1; ++i) {
      ...
      workers.launch(*this, process_chunk, chunk_first, cs, i);
    }

    delta = (chunk_size+1)*offset + chunk_size * (concurrency_degree_ - 1 - offset);
    const auto chunk_first = std::next(first, delta);
    process_chunk(chunk_first, sequence_size - delta, concurrency_degree_-1);
  } //Pool sync
    ...
    Transformer && transform_op, Combiner && combine_op) const
{
  ...
  
  {
    ...
    for (int i=0; i!=concurrency_degree_-1; ++i) {
      ...
      const auto chunk_firsts = iterators_next(firsts,delta);
      workers.launch(*this, process_chunk, chunk_firsts, cs, i);
    }
    ...
    Neighbourhood && neighbour_op) const
{
  ...
  
  {
    ...
    for (int i=0; i!=concurrency_degree_-1; ++i) {
      ...
      const auto chunk_firsts = iterators_next(firsts,delta);
      const auto chunk_first_out = std::next(first_out, delta);
      workers.launch(*this, process_chunk, chunk_firsts, cs, chunk_first_out);
    }
