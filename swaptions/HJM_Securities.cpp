...
void worker(backend opt){
  int size = nSwaptions;
  grppi::dynamic_execution ex = execution_mode(opt, nThreads);
		      		
  using namespace std;
  vector<int> index(size), w(size);
  iota(index.begin(), index.end(),0);
  map(ex, std::make_tuple(swaptions, index.begin()),size, swaptions, [](parm swaptions, int index)
	      							{
	 FTYPE pdSwaptionPrice[2];
	int iSuccess =  HJM_Swaption_Blocking(pdSwaptionPrice, swaptions.dStrike, 
		               swaptions.dCompounding, swaptions.dMaturity, 
		               swaptions.dTenor, swaptions.dPaymentInterval,
		               swaptions.iN, swaptions.iFactors, swaptions.dYears, 
		               swaptions.pdYield, swaptions.ppdFactors,
		               swaption_seed+index, NUM_TRIALS, BLOCK_SIZE, 0);
	assert(iSuccess == 1);
	swaptions.dSimSwaptionMeanPrice = pdSwaptionPrice[0];
	swaptions.dSimSwaptionStdError = pdSwaptionPrice[1];
	return swaptions;});
...
void * worker(void *arg){
 ...
 #ifdef ENABLE_OPENMP
 #pragma omp parallel for private(pdSwaptionPrice)
...
#if defined ENABLE_THREADS || defined ENABLE_OPENMP || defined ENABLE_GRPPI
...
#else
#ifdef ENABLE_GRPPI
  	backend opt = stringToBackend(argv[3]);
	worker(opt);
#else
	int threadID=0;
	worker(&threadID);
