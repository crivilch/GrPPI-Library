...
{
	grppi::dynamic_execution ex = execution_mode(opt, nthr);
	Point p0 = points->p[0];
  	int dim = points->dim;

	grppi::map(ex, points->p, points->num, points->p,[p0, dim](Point punto)
	      	{float distance = dist(punto, p0, dim);
   			 punto.cost = distance * punto.weight;
   			 punto.assign=0; return punto;});
	...
	*kcenter = 1;
	 ...
	 for(int i = 1; i < points->num; i++ )  {
		...
		(*kcenter)++;
		Point pi = points->p[i];
		grppi::map(ex, points->p, points->num, points->p,
				[i, pi, dim](Point punto) {float distance = dist(pi, punto, dim);
		      		if(distance*punto.weight < punto.cost){
	   				 punto.cost = distance * punto.weight;
	   				 punto.assign=i;} 
				return punto;}
	   			);
	...
	float totalcost = z*(*kcenter);
	Point p; p.cost = 0.0;
	Point out = grppi::reduce(ex, points->p, points->num, p, [](const Point & x, const Point & y){ Point w; w.cost = x.cost + y.cost; return w;});
	return totalcost+out.cost;
}
...
double pgain(long x, Points *points, double z, long int *numcenters, int pid, pthread_barrier_t* barrier){
  ...
  //my block
  long bsize = points->num/nthr;
  long k1 = bsize * pid;
  long k2 = k1 + bsize;
  if( pid == nthr-1 ) k2 = points->num;
  ...
  if( pid==0 )    { 
    work_mem = (double*) malloc(stride*(nthr+1)*sizeof(double));
    gl_cost_of_opening_x = 0;
    gl_number_of_centers_to_close = 0;
  }
  ...
  */
  ...
  if( pid== 0 ) memset(work_mem+nthr*stride,0,stride*sizeof(double));
  ...
  double* lower = &work_mem[pid*stride];
  //global *lower* fields
  double* gl_lower = &work_mem[nthr*stride];
...
float pkmedian(Points *points, long kmin, long kmax, long* kfinal, int pid, pthread_barrier_t* barrier){
  ...
  Point p0 = points->p[0];
  long pdim = points->dim;

  hiz = grppi::map_reduce(ex, points->p, points->num, 0.0, [p0, pdim](Point x){return dist(x,p0,pdim)*x.weight;},
  ...
  /* helps to guarantee correct # of centers at the end */
   ...
   for( int i = 0; i< points->num; i++ ) {
    ...
    while_arg_t* arg = new while_arg_t[nthr];
   ...
   for( int i = 0; i < nthr; i++ ) {
    ...
    grppi::map(ex, arg, nthr, arg,[barrier](while_arg_t & r){

		while(1) {
    /* first get a rough estimate on the FL solution */
    double lastcost = r.cost;
    r.cost = pFL(r.points, feasible, numfeasible,
	       r.z, &k, r.cost, (long)(ITER*r.kmax*log((double)r.kmax)), 0.1, r.pid, barrier);

    /* if number of centers seems good, try a more accurate FL */
    if (((k <= (1.1)*r.kmax)&&(k >= (0.9)*r.kmin))||
	((k <= r.kmax+2)&&(k >= r.kmin-2))) {

      /* may need to run a little longer here before halting without
	 improvement */
      r.cost = pFL(r.points, feasible, numfeasible,
		 r.z, &k, r.cost, (long)(ITER*r.kmax*log((double)r.kmax)), 0.001, r.pid, barrier);
    }

    if (k > r.kmax) {
      /* facilities too cheap */
      /* increase facility cost and up the cost accordingly */
      r.loz = r.z; r.z = (r.hiz+r.loz)/2.0;
      r.cost += (r.z-r.loz)*k;
    }
    if (k < r.kmin) {
      /* facilities too expensive */
      /* decrease facility cost and reduce the cost accordingly */
      r.hiz = r.z; r.z = (r.hiz+r.loz)/2.0;
      r.cost += (r.z-r.hiz)*k;
    }

    /* if k is good, return the result */
    /* if we're stuck, just give up and return what we have */
    if (((k <= r.kmax)&&(k >= r.kmin))||((r.loz >= (0.999)*r.hiz)) )
      { 
	break;
      }
#ifdef ENABLE_THREADS
...
#define __PARSEC_XSTRING(x) __PARSEC_STRING(x)
  ...
  char * timefile = argv[9];

  #ifdef ENABLE_GRPPI
  opt = stringToBackend(getenv("G_OPT"));
 #endif
#ifdef TBB_VERSION
